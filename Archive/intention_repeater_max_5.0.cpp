/*
    Intention Repeater MAX v5.0 (c)2020-2021 by Anthro Teacher aka Thomas Sweet.
...
void create_nesting_files() {
  ...
  cout << WHITE << "Intention Repeater Nesting Files Written." << endl;

  cout << "Be sure to have your intentions in the INTENTIONS.TXT file." << endl;

  cout << "To run, use --boostlevel 50, for example to use up to Nesting to 50 "
          "levels deep."
       << endl;

  cout << "When using --boostlevel 50, for example, it will ignore the "
          "--intent, and use \"NEST-50.TXT\" for the intent instead."
       << endl
       << endl;
}
...
void print_help() {
  ...
  cout << "Intention Repeater MAX v5.0 (c)2020-2021 by Anthro Teacher aka "
          "Thomas Sweet."
       << endl;
  ...
  cout << " i) --createnestingfiles or -p, example: --createnestingfiles"
       << endl;
  ...
  cout << "--colorhelp = List and show all available colors for text using the "
          "option --color."
       << endl;
  cout << "--createnestingfiles = Create the NEST- files required for boosting."
       << endl;
  cout << "--boostlevel = Set the level to boost the power (1-100). Use "
          "--createnestingfiles first to create the required NEST- files."
       << endl;
...
void create_hololink_files() {
  ...
  HOLOLINK_CONTENTS +=
      ...
      HSUPLINK_FILE + ".\r\n";
  HOLOLINK_CONTENTS += "\r\n";
  HOLOLINK_CONTENTS += HOLOSTONE_FILE +
...
int main(int argc, char **argv) {
  std::string intention, PROCESS_STATEMENT, process_intention, intention_value,
      ...
      param_freq, param_color, param_usehololink, param_boost_level,
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
      ...
      param_boost_level = argv[i + 1];

    } else if (!strcmp(argv[i], "-p") ||
               !strcmp(argv[i], "--createnestingfiles")) {
  ...
  if (param_boost_level != "0") {
    if (stoi(param_boost_level) > 100) {
      param_boost_level = "100";
    }

    if (stoi(param_boost_level) < 1) {
      param_boost_level = "1";
    }

    param_intention = "NEST-" + param_boost_level + ".TXT";
  }

  cout << "Intention is: " << param_intention << endl;
  ...
  cout << "Holo-Link framework originally created by Mystic Minds. This "
       ...
       << endl;
  cout << "Boosting through Nested Files and Intention Multiplying created by "
          "Anthro Teacher."
       << endl;
