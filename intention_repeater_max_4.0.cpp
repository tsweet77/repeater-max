/*
    Intention Repeater MAX v4.0 (c)2020-2021 by Anthro Teacher aka Thomas Sweet.
    Performance benchmark, enhancement and flags by Karteek Sheri.
    Holo-Link framework created by Mystic Minds.
    This implimentation of the Holo-Link by Anthro Teacher.
...
void print_help() {
  ...
  cout << "Intention Repeater MAX v4.0 (c)2020-2021 by Anthro Teacher aka Thomas Sweet." << endl;
  ...
  cout << "Holo-Link framework by Mystic Minds." << endl;
  ...
  cout << " h) --freemem or -e" << endl;
  cout << " i) --usehololink or -u" << endl;
  cout << " j) --createhololinkfiles or -x" << endl;
  cout << " k) --colorhelp or -n" << endl;
  cout << " l) --usemultiplier or -p" << endl;
  ...
  cout << "--usemultiplier = Specify YES or NO, whether to use the Multiplier functionality. Default = YES." << endl;
  cout << "--freemem = Tells you how much memory is free and exits." << endl;
...
void create_hololink_files() {
  ...
  cout << "Now you may now run with the --usehololink option." << endl;
...
int main(int argc, char ** argv) {
  std::string intention, PROCESS_STATEMENT, process_intention, intention_value, duration, param_usemultiplier, param_duration, param_intention, param_intention_2, param_timer, param_freq, param_color, param_usehololink, runtime_formatted, ref_rate, suffix_value = "HZ";
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if (!strcmp(argv[i], "-n") || !strcmp(argv[i], "--colorhelp")) {
      print_color_help();
      exit(EXIT_SUCCESS);
    }
    
    if (!strcmp(argv[i], "-d") || !strcmp(argv[i], "--dur")) {
    ...
    if (!strcmp(argv[i], "-m") || !strcmp(argv[i], "--imem")) {
    ...
    if (!strcmp(argv[i], "-i") || !strcmp(argv[i], "--intent")) {
    ...
    if (!strcmp(argv[i], "-p") || !strcmp(argv[i], "--usemultiplier")) {
      param_usemultiplier = argv[i + 1];
      std::transform(param_usemultiplier.begin(), suffix_value.end(), suffix_value.begin(), ::toupper);
    }
    
    if (!strcmp(argv[i], "-c") || !strcmp(argv[i], "--color")) {
      param_color = argv[i + 1];
      std::transform(param_color.begin(), param_color.end(), param_color.begin(), ::toupper);
    }
    
    if (!strcmp(argv[i], "-x") || !strcmp(argv[i], "--createhololinkfiles")) {
      create_hololink_files();
      exit(EXIT_SUCCESS);
    }
    
    if (!strcmp(argv[i], "-e") || !strcmp(argv[i], "--freemem")) {
      getFreeSystemMemory();
      exit(EXIT_SUCCESS);
    }
    
    if (!strcmp(argv[i], "-s") || !strcmp(argv[i], "--suffix")) {
  ...
  cout << "Intention Repeater MAX v4.0 (c)2020-2021 by Anthro Teacher aka Thomas Sweet." << endl;
  cout << "Performance benchmark, exponents and flags by Karteek Sheri." << endl;
  cout << "Holo-Link framework originally created by Mystic Minds. This implimentation by Anthro Teacher." << endl;
  ...
  if (param_freq == "0" || param_usemultiplier == "NO") //Only use multiplier if --freq flag is not set or specifically said not to use it.
  {
